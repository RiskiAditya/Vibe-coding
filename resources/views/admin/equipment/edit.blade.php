...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
    <h2 class="text-2xl font-bold text-gray-900 mb-6">Edit Alat</h2>
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('equipment.update', $equipment) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="stock" id="stock" value="{{ old('stock', $equipment->stock) }}" required min="1"
                ...
                <p class="mt-1 text-xs text-gray-500">Total: {{ $equipment->stock }}, Tersedia: {{ $equipment->available_stock }}, Dipinjam: {{ $equipment->stock - $equipment->available_stock }}</p>
                @error('stock')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="4"
                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $equipment->description) }}</textarea>
                @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <img src="{{ asset('storage/' . $equipment->image) }}" alt="{{ $equipment->name }}" class="mt-2 h-32 w-32 object-cover rounded-md">
            </div>
            @endif

            <div class="mb-6">
                <label for="image" class="block text-sm font-medium text-gray-700">Gambar Alat Baru</label>
        ...
        <form action="{{ route('equipment.destroy', $equipment) }}" method="POST" class="mt-4" onsubmit="return confirm('Apakah Anda yakin ingin menghapus alat ini?');">
            @csrf
            @method('DELETE')
