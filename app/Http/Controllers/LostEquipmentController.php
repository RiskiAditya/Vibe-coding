...
class LostEquipmentController extends Controller
{
    /**
     * Export lost equipment to Excel (CSV format).
     */
    public function exportExcel(Request $request)
    {
        ...
        $filename = 'barang-hilang-' . date('Y-m-d') . '.csv';
    ...
    /**
     * Display list of lost equipment from returns.
     */
    public function index(Request $request)
    {
        ...
        $lostItems = $query->orderBy('actual_return_date', 'desc')->paginate(20);
        ...
        // Chart data - Lost items by month (last 6 months)
        $monthlyData = Borrowing::where('status', 'returned')
            ->where('return_condition', 'hilang')
            ->whereNotNull('actual_return_date')
            ->where('actual_return_date', '>=', now()->subMonths(6))
            ->selectRaw('DATE_FORMAT(actual_return_date, "%Y-%m") as month, COUNT(*) as count')
            ->groupBy('month')
            ->orderBy('month')
            ->get();

        // Chart data - Lost items by category
        $categoryData = Borrowing::where('borrowings.status', 'returned')
            ->where('borrowings.return_condition', 'hilang')
        ...
        return view('admin.lost-equipment.index', compact('lostItems', 'stats', 'monthlyData', 'categoryData'));
    }
}
